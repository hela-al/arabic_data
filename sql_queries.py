...
songplay_table_create = ( "CREATE TABLE IF NOT EXISTS songplays(songplay_id SERIAL PRIMARY KEY NOT NULL,start_time float NOT NULL, user_id int NOT NULL, level varchar, song_id varchar, artist_id varchar, session_id int NOT NULL, location varchar, user_agent varchar)")
...
time_table_create = ("CREATE TABLE IF NOT EXISTS time( hour varchar PRIMARY KEY, day varchar, week varchar, month varchar, year varchar, weekday varchar)")
...
artist_table_insert=("INSERT INTO artists(artist_id, name, location, latitude, longitude) VALUES(%s,%s,%s,%s,%s) ON CONFLICT(artist_id) DO UPDATE SET location = EXCLUDED.location")


time_table_insert = ("INSERT INTO time( hour, day , week , month , year , weekday ) VALUES(%s,%s,%s,%s,%s,%s)ON CONFLICT(hour) DO NOTHING")
